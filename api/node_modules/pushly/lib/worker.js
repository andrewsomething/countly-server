...
PushlyWorker.prototype.startMessageStatusUpdater = function(message) {
	var self = this,
		f = function() {
			if (message.id in self.messages && (message.result.status & (M.Status.Sent | M.Status.Aborted)) === 0) {
...
PushlyWorker.prototype.updateMessage = function(message, immediate, error) {
	...
	if (immediate || !this.messagesStatusTimes || this.messagesStatusTimes[message.id] < (Date.now() - this.options.statusUpdatePeriod)) {
	...
	debug('Updated message in process %d: %j, %j, %j', process.pid, message, immediate, error);
};
