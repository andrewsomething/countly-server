...
APNS.prototype.connect = function () {
	...
	if (this.closed) {
		...
		var defer = q.defer();
		defer.fail(new Err(Err.CONNECTION, 'Connection closed'));
...
APNS.prototype.connectWithBackoff = function(){
	...
	if (this.closed) {
		debug('Skipped socket creation on closed connection');
		defer.fail(new Error('Connection closed'));
		return defer.promise;
	} else {
		...
		operation.attempt(function(attempt){
			...
			this.connect().delay(200).then(function(){
				...
				defer.fail(operation.mainError());
            }.bind(this));
		}.bind(this));
	}
	return defer.promise;
 ...
 */
 APNS.prototype.onSocketTimeout = function() {
	...
	if (this.socket.notifications.length) {
...
APNS.prototype.onSocketDrain = function(serviceBuffer) {
	...
	if (serviceBuffer === true && !this.runningAfterIO) {
		setImmediate(function() { 
			this.runningAfterIO = false; 
			this.service(); 
		}.bind(this));
		this.runningAfterIO = true;
	}
};
 ...
 * @private
 */
APNS.prototype.destoySocket = function() {
...
APNS.prototype.service = function() {
	...
	if (!this.socket) {
		if ((!this.connectDeferred || !this.connectDeferred.promise.isPending()) && this.notifications.length) {
...
APNS.prototype.abort = function (message) {
	var que = new FixedSizeDequeue(this.options.apnsNotificationCacheForErrorPurposes), notification;
	while (this.notifications.length) {
		notification = this.notifications.shift();
...
APNS.prototype.send = function (messageId, content, encoding, expiry, device, locale) {
	debug('Sending message id %j content %j encoding %j expiry %j device %j locale %j', messageId, content, encoding, expiry, device, locale);
	...
	// Expiry
	data.writeUInt32BE(expiry.getTime() / 1000, position);
	position += 4;
	...
	this.notifications.push([id, device, data, messageId]);
